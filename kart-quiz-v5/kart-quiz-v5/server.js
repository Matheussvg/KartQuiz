...
function startRace(room) {
  ...
  if (room.question) { clearTimeout(room.question.timer); room.question = null; }
...
wss.on("connection", ws => {
  ws.on("message", raw => {
    ...
    if (m.type === "create" && !p) return createRoom(ws, typeof m.name === 'string' ? m.name : 'Piloto');
    if (m.type === "join") {
      ...
      return joinRoom(ws, r, typeof m.name === 'string' ? m.name : 'Piloto', false);
    }
    if (!room) return;
    ...
    if (m.type === "pos") {
      if (room.state !== 'racing' || p.finished) return;
      if (![m.x,m.y,m.angle,m.prog,m.lap].every(Number.isFinite) || !Number.isInteger(m.lap) || m.lap < 1 || m.lap > LAPS + 1) return;
    ...
    if (m.type === "answer" && room.question && !p.finished) {
      const qs = room.question;
      if (Date.now() > qs.deadline || !Number.isInteger(m.idx) || m.idx < 0 || m.idx >= qs.q.options.length) return;
  ...
  ws.on("close", () => {
    ...
    if (room.players.size === 0) { room.state = 'closed'; clearTimeout(room.endTimer); clearInterval(room.cardTimer); if(room.question) clearTimeout(room.question.timer); rooms.delete(room.code); return; }
