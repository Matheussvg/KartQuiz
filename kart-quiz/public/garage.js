(()=>{
 const $=s=>document.querySelector(s),defaults={color:'#ff4d64',body:'sport',wheels:'standard',wing:'sport'};
 ...
 for(const [key,allowed] of Object.entries({body:['sport','buggy','classic'],wheels:['standard','wide','offroad'],wing:['none','sport','double']}))if(!allowed.includes(window.kartStyle[key]))window.kartStyle[key]=defaults[key];
 let renderer,scene,camera,kart,open=false;
 const fields={color:$('#kartColor'),body:$('#kartBody'),wheels:$('#kartWheels'),wing:$('#kartWing')};
